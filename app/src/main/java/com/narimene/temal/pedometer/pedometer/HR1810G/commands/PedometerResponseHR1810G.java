...
import static fr.semantic.ecare.app.android.components.sensor.pedometer.ConversionData.getDecimalTemperature;

/**
     * Class used to load Data from the Pedometer HR1755 J-style
     */

    public class PedometerResponseHR1810G extends PedometerResponse{




        public static PedometerResponseHR1810G newInstance(){
            return new PedometerResponseHR1810G();
        }
        ...
        private static OnLoadDataPedometerHR1810G mListenerHR1810G = null;

        public static OnLoadDataPedometerHR1810G getListenerHR1810G() {
            return mListenerHR1810G;
        }

        public static void setListenerHR1810G(OnLoadDataPedometerHR1810G mListener) {
            PedometerResponseHR1810G.mListenerHR1810G = mListener;
        }

        public static void setLastNecessarySampleStepsIsLoaded(boolean lastNecessarySampleStepsIsLoaded) {
            PedometerResponseHR1810G.lastNecessarySampleStepsIsLoaded = lastNecessarySampleStepsIsLoaded;
        }

        public static void setLastNecessarySampleSleepIsLoaded(boolean lastNecessarySampleSleepIsLoaded) {
            PedometerResponseHR1810G.lastNecessarySampleSleepIsLoaded = lastNecessarySampleSleepIsLoaded;
        }
            ...
            switch (commandReceived[indexEndMessages - 1 ]){
                ...
                case END_MESSAGE:
                    ...
                    if (mListenerHR1810G != null)
                        mListenerHR1810G.onLoadingStepsDaysIsFinish(globalListAllStepsActivityObject);
        ...
        public static void getDetailedStepsData(byte[] commandReceived, int mDaysAgo) {
            ...
            try {
                ...
                if (Tools.checkWithAcquisitionDate(detailedStepsDataList.get(detailedStepsDataList.size() - 1), mDaysAgo)){
                    ...
                    if (mListenerHR1810G != null)
                        mListenerHR1810G.onLoadingStepsIsFinish(detailedStepsDataList);
                }
            ...
            if (counterFrameReceivedFromDevice == FRAME_LENGTH_DETAILS_STEP && !lastNecessarySampleStepsIsLoaded){
                counterFrameReceivedFromDevice = 0;
                if (mListenerHR1810G != null)
                    mListenerHR1810G.onInsufficientLoadingSteps();
            }
        }
        ...
        public static void getDetailedSleepData(byte[] commandReceived, int mDaysAgo){
            ...
            try {
                ...
                if (Tools.checkWithAcquisitionDate(detailedSleepDataList.get(detailedSleepDataList.size() - 1), mDaysAgo)){
                    ...
                    if (mListenerHR1810G != null)
                        mListenerHR1810G.onLoadingSleepIsFinish(detailedSleepDataList);
                }
            } catch (ParseException e) {e.printStackTrace();}
            ...
            if (counterFrameReceivedFromDeviceSleep == FRAME_LENGTH_DETAILS_SLEEP && !lastNecessarySampleSleepIsLoaded){
                counterFrameReceivedFromDeviceSleep = 0;
                if (mListenerHR1810G != null)
                    mListenerHR1810G.onInsufficientLoadingSleep();
            }
        }
...
///------------------------------------------------------------------------------------------------------------------------
    ...
    // a GOLBAL STATIC ARRAY OF LAST UDATED TEMERATURES
    public static List<MeasureModel> updatedGlobalTemperatureMeasures = new ArrayList<MeasureModel>();

    // Filtering by TIMESTAMP to Update the static Global Array of the last temperature measurements

    public static Boolean checkIfNewTemperatureMeasuresList (List<MeasureModel> mtemperaturesList) {

        if (mtemperaturesList.size() != 0 && mtemperaturesList.get(0).getDate() != updatedGlobalTemperatureMeasures.get((updatedGlobalTemperatureMeasures.size())-1).getDate())  {

            for (int i = updatedGlobalTemperatureMeasures.size(); i< mtemperaturesList.size(); i++){
                for ( int j=0 ; j< mtemperaturesList.size(); j++){
                    updatedGlobalTemperatureMeasures.add(mtemperaturesList.get(j));
                }
            }
            if (mListenerHR1810G != null)
                mListenerHR1810G.onLoadingTemperatureIsFinish(updatedGlobalTemperatureMeasures);
        }return true;
    }
    ...
    public static void getTemperatureMeasure (List<Byte> commandReceived, String patientID) throws ParseException {

         List<MeasureModel> temperatureModelMeasures = new ArrayList<>();
        // make sure to get all temperatures Values , So we repeat reading Byte until we get the 96 -1 bits
        // in the end of Byte Array
        for (int i=0; i< commandReceived.size() ; i +=11){

            if (commandReceived.get(i) == 96 && commandReceived.get(i+1) == -1 ){

                Log.i("Data == ", temperatureModelMeasures.size() + " ");

                int count = 0;

                for (MeasureModel data : temperatureModelMeasures){

                    try {
                        data.setIdServer("MEASURE_" + System.currentTimeMillis() + "_" + count);
                        MeasureService.add(data);
                        count++;
            ...
            MeasureModel mTemperaturevalue = new MeasureModel();

        // get TimeStamp in long

            int Year = byteHexToInteger(commandReceived.get(i+3));
            int Month =  (byteHexToInteger(commandReceived.get(i+4)));
            int Day = (byteHexToInteger(commandReceived.get(i+5)));
            int Hour =  (byteHexToInteger(commandReceived.get(i+6)));
            int Minutes = (byteHexToInteger(commandReceived.get(i+7)));
            int Second = (byteHexToInteger(commandReceived.get(i+8)));

            // Check Time
            String time = ConversionData.dateToString(Year, Month, Day, Hour,Minutes, Second );
            Log.i("TEMPERATURE ==> Time : ", time  );

            long timeStamp = ConversionData.getTimeStamp(ConversionData.dateToString(Year, Month, Day, Hour,Minutes, Second ));
            Log.i("TEMPERATURE ==> TimeStamp : ", timeStamp + " ");

            mTemperaturevalue.setDate(timeStamp);



            // set Temperature in float and timeStamp  by using the MeasureModel (value)

            mTemperaturevalue.setValue(getDecimalTemperature(Integer.parseInt(String.valueOf(commandReceived.get(i+9)))));
            Log.i ("TEMPERATURE", "==> temperature Value  : " + getDecimalTemperature(Integer.parseInt(String.valueOf(commandReceived.get(i+9)))) );
            mTemperaturevalue.setType(MeasureType.TEMPERATURE);
            mTemperaturevalue.setPeriodId("");

            PatientModel patient = PatientActivity.getSelectedPatient();
            String mPatientId = patient.getIdServer();

            mTemperaturevalue.setPatientId(mPatientId);
            mTemperaturevalue.setCommentIdList("");
            mTemperaturevalue.setComment("");
            mTemperaturevalue.setSynchronized(false);
            mTemperaturevalue.setPeriodUpdated(false);

            // Insert Temperature Details in Data Base

            temperatureModelMeasures.add(mTemperaturevalue);
        ...
        //==============================================================================================
        // Methods for Real Time Data with Temperature activated
        //==============================================================================================

        public static void getRealTimeTemperature(byte[] commandReceived){
            RealTimeData data = RealTimeData.getInstance();
            ...
            //int byteToIntTemp = Integer.parseInt(String.valueOf(bytes.get(i+9)));
            //mTemperatureData.setFloatValueTemperature(getDecimalTemperature(byteToIntTemp));

            int byteToIntTempRealTime = Integer.parseInt(String.valueOf(commandReceived[22]));
            data.setTemperature(getDecimalTemperature(byteToIntTempRealTime));


            //data.setTemperature(commandReceived[23]);
